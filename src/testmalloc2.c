...
//	mlt_reset();
	...
	{
		struct structProcStat * __attribute__((unused)) procStat = (struct structProcStat *) malloc (sizeof(struct structProcStat));	// Simulate a memory leak!
		mlt_analyze(0);												// Analyze the simulated memory leak
	}
	free(procStat1);
	//printf("testmalloc2: 2nd free(procStat1)\n");									// Simulate double call to free()
	//free(procStat1);
...
//	mlt_reset();
	...
	for (i=0; i<MLT_INCREASE_THRESHOLD+2; i++)
	{
		struct structProcStat * __attribute__((unused)) procStat = (struct structProcStat *) malloc (sizeof(struct structProcStat[4]));	// Simulate a memory leak!
		mlt_analyze(0);												// Analyze the simulated memory leak
	}
	free(procStat2);
